...
struct node
{
    ...
    static node_t* make_inner_sr_n(count_t n, relaxed_t* r)
    {
        return static_if<embed_relaxed, node_t*>(
            ...
            [&] (auto) {
                ...
                NOTE("make_inner_sr_n: from:%p to:%p\n", p, (char*)p+node_t::sizeof_inner_r_n(n));
                assert(r->d.count >= n);
                node_t::refs(r).inc();
                p->impl.d.data.inner.relaxed = r;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_inner_sr_e(edit_t e, relaxed_t* r)
    {
        return static_if<embed_relaxed, node_t*>(
            ...
            [&] (auto) {
                ...
                NOTE("make_inner_sr_e: from:%p to:%p\n", p, (char*)p+node_t::max_sizeof_inner_r);
                node_t::refs(r).inc();
                p->impl.d.data.inner.relaxed = r;
                node_t::ownee(p) = e;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* do_copy_inner_sr(node_t* dst, node_t* src, count_t n)
    {
        if (embed_relaxed)
            ...
            NVM_PERSIST(dst->inner(), (char*)(dst->inner()+n-1) - (char*)dst->inner());
            return dst;
        }
    }
    ...
    static node_t* copy_leaf(node_t* src, count_t n)
    {
        ...
        try {
            ...
            NOTE("copy_leaf: dst->leaf:%p to :%p\n", dst->leaf(), (char*) dst->leaf() + n);
            NVM_PERSIST(dst->leaf(), (char*)(dst->leaf()+n-1) - (char*)dst->leaf());
        } catch (...) {
                             ...
                             node_t* src2, count_t n2)
    {
        ...
        NOTE("copy_leaf_n1_n2: dst->leaf:%p to :%p\n", dst->leaf(), (char*) dst->leaf()+n1+n2);
        NVM_PERSIST(dst->leaf(), (char*)(dst->leaf()+n1+n2-1) - (char*)dst->leaf());
        return dst;
    }
    ...
    static node_t* copy_leaf_e(edit_t e, node_t* src, count_t idx, count_t last)
    {
        ...
        NOTE("copy_leaf_e: dst->leaf:%p to :%p\n", dst->leaf(), (char*) dst->leaf()+last-idx);
        NVM_PERSIST(dst->leaf(), (char*)(dst->leaf()+last-idx-1) - (char*)dst->leaf());
        return dst;
    }
