...
//					help();
				...
				break;
			// case 's':
			// 	this->stm = atoi(optarg);
			// 	if(stm>=stms.size()){
			// 		fprintf(stderr, "Invalid stm (-s) option.\n");
			// 		help();
			// 		errexit("");
			// 	}
			// 	break;
...
*/
void PinConfig::setEnv(std::string key, std::string value){
	if(verbose){
		std::cout <<"setEnv: "<< key << " = \"" << value << "\"\n";
	}
	environment[key]=value;
}
bool PinConfig::checkEnv(std::string key){
	if(verbose){
		std::cout <<"checkEnv: "<< key << "\n";
	}
	return environment[key]!="" && environment[key]!="0";
}
std::string PinConfig::getEnv(std::string key){
	if(verbose){
		std::cout <<"getEnv: "<< key << "\n";
	}
...
void PinConfig::buildDFSAffinity_helper(hwloc_obj_t obj){
...
void PinConfig::buildDFSAffinity(){
	buildDFSAffinity_helper(hwloc_get_root_obj(topology));
}

int PinConfig::buildDefaultAffinity_findCoresInSocket(hwloc_obj_t obj, std::vector<hwloc_obj_t>* cores){
...
int PinConfig::buildDefaultAffinity_buildPUsInCores(std::vector<hwloc_obj_t>* cores){
		int coreIndex = 0;
		int coresFilled = 0;
...
int PinConfig::buildDefaultAffinity_findAndBuildSockets(hwloc_obj_t obj){
...
void PinConfig::buildDefaultAffinity(){
...
void PinConfig::buildSingleAffinity_helper(hwloc_obj_t obj){
	if(obj->type==HWLOC_OBJ_PU){
...
void PinConfig::buildSingleAffinity(){
