...
public:
	...
	struct ret
	{
		...
		ret(bool _found, KV_entry_ptr_t _ptr) : found(_found), ptr(_ptr), level_idx(0), bucket_idx(0),
...
RETRY_INSERT:
		...
		if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT)
		{
			...
			if(check_duplicate){   // check_duplicate is set to false when worker thread has inserted the KV successfully 
				return ret();
			}else{
				return ret(true);
			}
