...
d4<-subset(montagedata,ds=="NVTraverse")
...
lindata$ds <- factor(lindata$ds, levels=c("DRAM (T)", "nbMontage", "Montage", "NVTraverse"))
...
linchart<-ggplot(data=lindata,
  ...
  coord_cartesian(xlim = c(-2, 60), ylim = c(y_range_down,y_range_up))+
  ...
  theme(axis.title.x = element_text(hjust =-0.32,vjust = 12.7,margin = margin(t = 15, r = 0, b = 10, l = 0)))
