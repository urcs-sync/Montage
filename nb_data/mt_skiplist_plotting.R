...
tests<-c("g50i25r25_thread")
for (t in tests){
read.csv(paste("./maps_",t,".csv",sep=""))->montagedata

montagedata$ds<-as.factor(gsub("nbMontageLfSkipList","nbMontage",montagedata$ds))
montagedata$ds<-as.factor(gsub("PLockfreeSkipList","Izraelevitz",montagedata$ds))
montagedata$ds<-as.factor(gsub("MontageLfSkipList","Montage",montagedata$ds))
montagedata$ds<-as.factor(gsub("NVMLockfreeSkipList","NVM (T)",montagedata$ds))
montagedata$ds<-as.factor(gsub("NVTLockfreeSkipList","NVTraverse",montagedata$ds))
montagedata$ds<-as.factor(gsub("LockfreeSkipList","DRAM (T)",montagedata$ds))
...
d6<-subset(montagedata,ds=="NVTraverse")
...
ggsave(filename = paste("./skiplists_",t,".pdf",sep=""),linchart,width=4, height = 4, units = "in", dpi=300, title = paste("skiplists_",t,".pdf",sep=""))
}
# width=15.95
