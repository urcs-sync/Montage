    ...
    bool insert(std::string key, std::string val, int tid) {
        ...
        while (1) {
            ...
            if ( val_less(key, leaf_key)) {
                ...
                MEM_BARRIER;
            #endif
            node_t* result = NVT_CASV(child_addr, ADDRESS(leaf), ADDRESS(new_internal));
...
//=====================================================
    ...
    bool_t bst_cleanup(const int tid, const std::string& key) {
        ...
        if (NVT_CASV(succ_addr, ADDRESS(successor), (node_t*) UNTAG(sibl)) == (node_t*) ADDRESS(successor)) {
...
class NVMNatarajanTreeFactory : public RideableFactory{
    Rideable* build(GlobalTestConfig* gtc){
        return new NVMNatarajanTree(gtc);
    }
};
