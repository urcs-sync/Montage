...
public:
    NVMLockfreeSkipList(int task_num) : tracker(task_num + 1, 100, 1000, true) {
        ...
        background_thread = std::move(std::thread(&NVMLockfreeSkipList::bg_loop, this, bg_tid));
    };
    ~NVMLockfreeSkipList(){
...
int NVMLockfreeSkipList<K,V>::bg_trav_nodes(int tid){
    ...
    while (nullptr != next) {
        ...
        if (nullptr != node->payload.ptr.load() && (void *) node != (void *) node->payload.ptr.load()) {
            ++bg_non_deleted;
        }
        prev = node;
        node = next;
...
template<class K, class V>
void NVMLockfreeSkipList<K,V>::bg_help_remove(Node *prev, Node *node, int tid){
...
template<class K, class V>
int NVMLockfreeSkipList<K,V>::internal_finish_contains(const K& key, Node *node, Payload *node_payload, optional<V>& ret_value){
    int result = 0;
...
template<class K, class V>
int NVMLockfreeSkipList<K,V>::internal_finish_delete(const K& key, Node *node, Payload *node_payload, optional<V>& ret_value, int tid){
    int result = -1;
...
template<class K, class V>
int NVMLockfreeSkipList<K,V>::internal_finish_insert(const K& key, V &val, Node *node, Payload *node_payload, Node *next, Payload *&lazy_payload){
    int result = -1;
    Node *new_node, *temp;
...
template<class K, class V>
bool NVMLockfreeSkipList<K,V>::internal_do_operation(operation_type optype, const K& key, optional<V>& val, optional<V>& ret_value, int tid){
